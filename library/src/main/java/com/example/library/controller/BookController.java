...
import com.example.library.util.apiDocs.BookControllerDoc;
...
public class BookController {
    ...
    @BookControllerDoc.UpdateBookDoc
    public ResponseEntity<EntityIdDto> updateBook(@PathVariable(value = RestConstants.ID) Long id,
    ...
    @BookControllerDoc.GetUserBooksDoc
    ...
    @PreAuthorize("hasAnyAuthority(@Role.ADMIN_NAME)")
    @GetMapping
    @BookControllerDoc.GetAllBooksDoc
    ...
    @BookControllerDoc.DeleteBookDoc
    public String deleteBook(@PathVariable(value = RestConstants.ID) Long id) {
        try {
            return bookService.deleteBook(id);
        } catch (DataIntegrityViolationException ex) {
            throw new RuntimeException("The book with id: " + id + " is linked to other entities and cannot be deleted!");
        }
    }
}
